export default {
  localBrowser: {
    ...
    "copyPairing": "复制配对链接",
    "copied": "已复制",
    "windowHint": "任务在 Chrome 的 DXZL 标签组内运行。借用已有标签页需确认。",
    "preview": "本机浏览器任务预览",
    "waiting": "等待任务页面",
  ...
  integrations: {
    ...
    claw: {
      title: 'DXZL Skill',
      subtitle: '通过 DXZL REST API 导入文档并执行混合检索（向量 + 关键词）。适用于上传文件/URL/Markdown 到知识库、跨库检索与浏览知识内容。',
      ...
      installCtaHint: '安装 DXZL Skill · 将在新标签页打开',
    ...
    chrome: {
      title: '知识管理助手',
      subtitle: '配合 DXZL 自建服务使用：在任意网页侧边栏提问、剪藏内容、Markdown 速记，将浏览中的知识沉淀到你的知识库。',
      ...
      steps: {
        ...
        port: {
          title: '桌面版配置固定端口（推荐）',
          desc: '使用 DXZL 桌面版时，在 API 信息中设置固定端口（如 37841），避免每次启动后地址变化导致插件断连。'
        },
        api: {
          title: '获取 API 凭证',
          desc: '在「设置 → API 信息」中复制 API Key 与 API 地址。'
    ...
    api: {
      ...
      signedFlowDetail: 'HMAC 密钥仅保存在 DXZL 与你的可信后端，切勿放入请求或下发给浏览器。请求头里传的是用该密钥签出的 JWT 字符串（每次签发或过期后会变），不是密钥本身。JWT 须包含 sub（外部用户 ID）、tenant_id、aud=weknora、exp（有效期不超过 24 小时）。',
      ...
      hmacSecret: 'HMAC 密钥',
      hmacSecretDesc: '与 DXZL 保存的同一份密钥；仅用于后端签发 JWT，不要作为请求头发送。',
  ...
  embedPublish: {
    ...
    originsHint: '填写允许嵌入的宿主网站来源（A），不是 DXZL 地址（B）。每行一个完整 Origin，如 https://shop.example.com；至少一项，支持 *.example.com 子域通配',
    ...
    webhookUrlDesc: '可选。访客发消息与助手回复完成时，DXZL 会向该 HTTPS 地址 POST 事件（message_sent / message_received）。',
  ...
  auth: {
    ...
    registerSubtitle: '创建账户并开始使用 DXZL',
  ...
  envVarSettings: {
    title: '沙箱密钥',
    description: '给技能和沙箱用的个人密钥，不是 DXZL 的系统或部署配置。',
    helpAria: '沙箱密钥说明',
  ...
  settings: {
    ...
    sandbox: {
      ...
      dockerHostRisk: '留空或 unix:// 会使用 DXZL 所在机器的 Docker 守护进程，权限等同该机 root，只适合私有化单机。多套空间共用同一主机时请改用 Cube 或 E2B。远程 tcp:// 必须填写 TLS 证书目录。',
      addConfig: '添加沙箱',
      ...
      weknoraTemplateTitle: 'DXZL 标准模板',
      weknoraDockerImage: 'DXZL 标准镜像',
      weknoraDockerImageHint: '每次会话独占一个长驻容器，脚本、shell 命令与文件都在同一个容器内，会话结束或空闲超时后回收。',
      weknoraTemplateOverview: 'DXZL 负责提供标准运行环境。连接集群后会自动发现模板，缺少标准模板时自动创建。',
      weknoraTemplateDescription: '包含 DXZL 技能所需的 Python、Node.js、CLI 工具、工作目录和非 root 执行用户。',
      recommendedTag: '推荐',
      ...
      standardTemplateProvisioning: '已开始在集群中创建 DXZL 标准模板，请稍后刷新查看状态',
      ...
      templateUntaggedHint: '构建已完成，但没有构建带 default 标签，创建沙箱时无法解析。请在 E2B 删除该模板，刷新后 DXZL 会重新构建。',
      templateFailedReason: '构建失败：{reason}',
      noTemplates: '当前集群未返回可用模板。',
      weknoraStandardTemplate: 'DXZL 标准模板',
      createStandardTemplate: '创建',
      createStandardTemplateHint: '按当前连接配置构建，包含 DNS。之后改配置可在卡片上重建。',
      weknoraDesktopTemplate: 'DXZL 桌面模板',
      createDesktopTemplate: '创建',
      createDesktopTemplateHint: '从官方桌面镜像构建 XFCE 图形桌面。体积明显大于标准模板，仅在需要图形界面时创建。',
      replaceStandardTemplate: '重建',
      replaceStandardTemplateConfirm: '将用当前配置（含 DNS）重建 DXZL 标准模板。新模板就绪前不会删除仍可启动的旧模板。',
      desktopTemplateProvisioning: '已开始在集群中创建 DXZL 桌面模板，请稍后刷新查看状态',
      desktopTemplateReplaced: '已删除原桌面模板并开始重建，请等待状态就绪。标准（CLI）模板不受影响。',
      desktopTemplateTag: '桌面',
      replaceDesktopTemplateConfirm: '将用当前配置（含 DNS）重建 DXZL 桌面模板。新模板就绪前不会删除仍可启动的旧桌面模板，也不会删除标准（CLI）模板。',
      ...
      cubeDnsServersHelp: '可选。写入 DXZL 标准模板的 nameserver（须为 IP）。留空则使用集群默认（常见 119.29.29.29）。私网或云上 UDP 53 出不去时，填 Cube 宿主机 /etc/resolv.conf 里能用的地址，并避开 10/8、172.16/12、192.168/16。已有标准模板需在模板卡片上点「重建」才会生效。',
      ...
      dockerTlsCertPathHelp: 'DXZL 所在主机上包含 ca.pem、cert.pem、key.pem 的目录。远程守护进程必填，证书不入库，由部署方挂载。',
      dockerIdleTtl: '空闲回收（秒）',
      dockerIdleTtlHelp: 'Docker 守护进程本身没有空闲超时。容器多久没有执行任何命令就会被 DXZL 回收，会话继续时重建。留空按 1800 秒。',
  ...
  newUserGuide: {
    ...
    steps: {
      ...
      knowledge: {
        title: '创建你的知识库',
        desc: '知识库是一切的起点：上传文档、网页或 FAQ，DXZL 会自动解析并建立索引。点击这里进入知识库。'
      },
      welcome: {
        title: '欢迎使用 DXZL',
